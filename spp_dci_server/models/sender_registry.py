...
"""DCI Sender Registry for signature verification.

This model inherits from spp.api.client to share:
- organization_type for consent matching
- legal_basis for GDPR compliance
- rate_limit fields for API throttling

DCI-specific fields (sender_id, public_key, algorithm) are added here.
"""
...
class DCISenderRegistry(models.Model):
    """DCI Sender Registry.

    Inherits from spp.api.client to share organization_type and consent features.
    Stores public keys of external DCI senders for signature verification.
    Allows fetching and updating public keys from sender JWKS endpoints.
    """

    _name = "spp.dci.sender.registry"
    _inherit = "spp.api.client"
    _description = "DCI Sender Registry"
    _order = "name"

    # Override auth_type default to HTTP Signature for DCI clients
    ...
    # ========================================================================
    # DCI-SPECIFIC FIELDS
    # These extend the base spp.api.client with DCI protocol requirements.
    # ========================================================================

    sender_id = fields.Char(
        required=True,
        string="Sender ID",
        help="DCI sender identifier (e.g., 'crvs.national.gov')",
        index=True,
    )
    ...
    jwks_url = fields.Char(
        string="JWKS URL",
        help="URL to sender's JWKS endpoint (/.well-known/jwks.json)",
    )
    ...
    notes = fields.Text(
        string="Notes",
        help="Additional notes about this sender",
    )
    ...
    auto_approve = fields.Boolean(
        ...
        help="Automatically confirm subscriptions from this sender without manual review",
    )

    # Rate limiting tracking fields
    # Note: rate_limit_per_minute and rate_limit_per_day are inherited from spp.api.client
    request_count_today = fields.Integer(
        default=0,
        readonly=True,
        string="Requests Today",
        help="Number of API requests made today (reset daily at midnight)",
    )
    request_count_minute = fields.Integer(
        default=0,
        readonly=True,
        string="Requests This Minute",
        help="Number of API requests in the current minute window",
    )
    last_request_minute = fields.Datetime(
        readonly=True,
        string="Last Request Time",
        help="Timestamp of the last API request (used for per-minute rate limiting)",
    )
    last_request_reset = fields.Date(
        readonly=True,
        string="Last Daily Reset",
        help="Date when daily counter was last reset",
    )

    @api.constrains("sender_id")
    def _check_sender_id_unique(self):
        """Ensure sender_id is unique across all sender registry entries."""
        for record in self:
            if record.sender_id:
                ...
                if duplicate:
                    raise ValidationError(
                        _("Sender ID must be unique. A registry entry already exists for this sender.")
                    )
    ...
    def action_fetch_public_key(self):
        """Fetch public key from sender's JWKS endpoint (button action).
        ...
        try:
            ...
            _logger.error(
                "Failed to fetch public key for sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
                exc_info=True,
    ...
    def fetch_public_key(self):
        """Fetch public key from sender's JWKS endpoint.
        ...
        if not self.jwks_url:
            raise UserError(_("JWKS URL not configured for sender %s") % self.sender_id)

        _logger.info(
            "Fetching public key for sender %s from %s",
            self.sender_id,
            self.jwks_url,
        )
        ...
        try:
            ...
            if not matching_key:
                raise UserError(
                    _("No matching key found in JWKS for sender %s. Expected kid to start with '%s|'")
            ...
            _logger.info(
                "Successfully fetched public key for sender %s (algorithm: %s)",
                self.sender_id,
                algorithm,
            )
            ...
            _logger.error(
                "HTTP error fetching JWKS for sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
            )
    ...
    def get_verifier(self):
        """Return a DCIVerifier instance for this sender.
        ...
        if not self.public_key:
            raise UserError(
                _("No public key available for sender %s. Please fetch the public key first.") % self.sender_id
            )

        if not self.algorithm:
            raise UserError(_("Algorithm not set for sender %s") % self.sender_id)
    ...
    def get_by_sender_id(self, sender_id):
        """Lookup sender by sender_id.

        Args:
            sender_id: DCI sender identifier

        Returns:
            spp.dci.sender.registry: Sender record or empty recordset
        """
        return self.search([("sender_id", "=", sender_id), ("active", "=", True)], limit=1)

    @api.model
    def _cron_reset_daily_rate_limits(self):
        """Reset daily rate limit counters for all senders.

        Called by scheduled action (cron) at midnight to reset daily counters.
        Uses direct SQL UPDATE for performance with large sender registries.
        """
        today = fields.Date.today()

        _logger.info("Running daily rate limit counter reset for date: %s", today)

        # Reset all daily counters using SQL for performance
        # Only reset if last_request_reset is not today (prevents duplicate resets)
        self.env.cr.execute(
            """
            UPDATE spp_dci_sender_registry
            SET request_count_today = 0,
                last_request_reset = %s
            WHERE last_request_reset IS NULL
               OR last_request_reset < %s
            """,
            (today, today),
        )

        rows_updated = self.env.cr.rowcount
        _logger.info("Reset daily rate limit counters for %d sender(s)", rows_updated)

        # Invalidate cache to reflect the updates
        self.invalidate_model(["request_count_today", "last_request_reset"])
