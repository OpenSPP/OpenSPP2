...
"""Common test utilities for DCI Client CRVS module."""
...
class CRVSClientCommon(TransactionCase):
    """Base test class with shared utilities for CRVS Client tests."""
    ...
    def setUpClass(cls):
        ...
        # Create test keypair for CRVS sender (registry side)
        ...
        # Test CRVS sender configuration
        cls.test_sender_id = "crvs.national.gov"
        cls.test_key_id = "2024-test-key"
        cls.test_algorithm = "ed25519"

        # Models
        cls.CRVSSender = cls.env["spp.dci.crvs.sender"]

    @classmethod
    def create_test_crvs_sender(
        cls,
        sender_id=None,
        name=None,
        public_key=None,
        ...
        """Helper to create a test CRVS sender registry entry.
        ...
        Returns:
            spp.dci.crvs.sender: Created sender record
        """
        sender_id = sender_id or cls.test_sender_id
        return cls.CRVSSender.create(
            {
                "name": name or f"Test CRVS Sender {sender_id}",
                "sender_id": sender_id,
    ...
    def create_signed_envelope(
        ...
        """Helper to create a signed DCI envelope for testing CRVS callbacks.

        Args:
            sender_id: Sender ID (defaults to cls.test_sender_id)
            receiver_id: Receiver ID
            action: DCI action (default: 'notify' for callbacks)
            message: Message payload (defaults to CRVS birth event)
        ...
        header_model = DCIMessageHeader(
            version="1.0.0",
            message_id="crvs-callback-001",
            message_ts=datetime.now(UTC),
            action=action,
            sender_id=sender_id,
            sender_uri="https://crvs.national.gov/api/v1",
        ...
        # Sign the message using CRVS sender's private key
        signer = DCISigner(
