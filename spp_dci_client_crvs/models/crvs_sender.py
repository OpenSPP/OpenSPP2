...
"""CRVS Sender Registry for signature verification.

This model stores trusted CRVS registry public keys for verifying
incoming callback notifications. Similar to spp.dci.sender.registry
but specifically for CRVS client-side verification.
"""
...
class CRVSSenderRegistry(models.Model):
    """CRVS Sender Registry.

    Stores public keys of trusted CRVS registries for signature verification
    of incoming callback notifications.
    """

    _name = "spp.dci.crvs.sender"
    _description = "CRVS Sender Registry"
    _order = "name"

    name = fields.Char(
        required=True,
        string="Name",
        help="Friendly name for this CRVS registry",
    )
    sender_id = fields.Char(
        required=True,
        string="Sender ID",
        help="DCI sender identifier for the CRVS registry (e.g., 'crvs.national.gov')",
        index=True,
    )
    ...
    jwks_url = fields.Char(
        string="JWKS URL",
        help="URL to CRVS registry's JWKS endpoint (/.well-known/jwks.json)",
    )
    ...
    notes = fields.Text(
        string="Notes",
        help="Additional notes about this CRVS registry",
    )

    @api.constrains("sender_id")
    def _check_sender_id_unique(self):
        """Ensure sender_id is unique across all CRVS sender entries."""
        for record in self:
            if record.sender_id:
                ...
                if duplicate:
                    raise ValidationError(_("Sender ID must be unique. A CRVS sender already exists with this ID."))
    ...
    def action_fetch_public_key(self):
        """Fetch public key from CRVS registry's JWKS endpoint (button action).
        ...
        try:
            ...
            _logger.error(
                "Failed to fetch public key for CRVS sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
                exc_info=True,
    ...
    def fetch_public_key(self):
        """Fetch public key from CRVS registry's JWKS endpoint.
        ...
        if not self.jwks_url:
            raise UserError(_("JWKS URL not configured for CRVS sender %s") % self.sender_id)

        _logger.info(
            "Fetching public key for CRVS sender %s from %s",
            self.sender_id,
            self.jwks_url,
        )
        ...
        try:
            ...
            _logger.info(
                "Successfully fetched public key for CRVS sender %s (algorithm: %s)",
                self.sender_id,
                algorithm,
            )
            ...
            _logger.error(
                "HTTP error fetching JWKS for CRVS sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
            )
    ...
    def _jwk_to_pem(self, jwk):
        ...
        try:
            if kty == "OKP" and jwk.get("crv") == "Ed25519":
                ...
                # Decode base64url (add padding if needed)
                # Use (4 - len % 4) % 4 to correctly handle cases where len is already a multiple of 4
                n_bytes = base64.urlsafe_b64decode(n + "=" * ((4 - len(n) % 4) % 4))
                e_bytes = base64.urlsafe_b64decode(e + "=" * ((4 - len(e) % 4) % 4))
    ...
    def get_verifier(self):
        """Return a DCIVerifier instance for this CRVS sender.
        ...
        if not self.algorithm:
            raise UserError(_("Algorithm not set for CRVS sender %s") % self.sender_id)
    ...
    def get_by_sender_id(self, sender_id):
        """Lookup CRVS sender by sender_id.

        Args:
            sender_id: DCI sender identifier

        Returns:
            spp.dci.crvs.sender: Sender record or empty recordset
        """
