...
class TestStatisticRegistry(TransactionCase):
    """Tests for spp.aggregation.statistic.registry."""

    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.stat_registry = cls.env["spp.aggregation.statistic.registry"]
    ...
    def test_compute_unknown_statistic(self):
        ...
        with self.assertLogs("odoo.addons.spp_aggregation.models.statistic_registry", level="WARNING") as log:
...
class TestStatisticRegistryIntegration(TransactionCase):
    """Integration tests for statistic registry with spp.statistic and spp.cel.variable."""
    ...
    def test_compute_from_statistic_model(self):
        """Test computing statistic from spp.statistic via variable."""
    ...
    def test_list_available_includes_statistics(self):
        """Test that list_available includes spp.statistic records."""
        if "spp.statistic" not in self.env:
            self.skipTest("spp_statistic module not installed")
        if "spp.cel.variable" not in self.env:
            self.skipTest("spp_cel module not installed")
