...
class SPPDMSDirectory(models.Model):
    ...
    active = fields.Boolean(default=True, help="Uncheck to archive the directory")
    ...
    def action_spp_dms_directories_all_directory(self):
        ...
        base_domain = literal_eval(action["domain"].strip())
        new_domain = [("parent_id", "child_of", self.id)]
        if base_domain:
            domain = Domain(base_domain) & Domain(new_domain)
            action["domain"] = domain.to_list()
        else:
    ...
    def action_spp_dms_files_all_directory(self):
        ...
        base_domain = literal_eval(action["domain"].strip())
        new_domain = [("directory_id", "child_of", self.id)]
        if base_domain:
            domain = Domain(base_domain) & Domain(new_domain)
            action["domain"] = domain.to_list()
        else:
