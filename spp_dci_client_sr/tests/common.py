...
"""Common test utilities for DCI Client SR module."""
...
class SRClientCommon(TransactionCase):
    """Base test class with shared utilities for SR Client tests."""
    ...
    def setUpClass(cls):
        ...
        # Create test keypair for SR sender (registry side)
        ...
        # Test SR sender configuration
        cls.test_sender_id = "sr.national.gov"
    ...
    def create_test_sr_sender(
        ...
        """Helper to create a test SR sender registry entry.
        ...
        Returns:
            spp.dci.sr.sender: Created sender record
        """
        sender_id = sender_id or cls.test_sender_id
        return cls.SRSender.create(
            {
                "name": name or f"Test SR Sender {sender_id}",
                "sender_id": sender_id,
    ...
    def create_signed_envelope(
        ...
        """Helper to create a signed DCI envelope for testing SR callbacks.

        Args:
            sender_id: Sender ID (defaults to cls.test_sender_id)
            receiver_id: Receiver ID
            action: DCI action (default: 'notify' for callbacks)
            message: Message payload (defaults to SR birth event)
        ...
        header_model = DCIMessageHeader(
            version="1.0.0",
            message_id="sr-callback-001",
            message_ts=datetime.now(UTC),
            action=action,
            sender_id=sender_id,
            sender_uri="https://sr.national.gov/api/v1",
        ...
        # Sign the message using SR sender's private key
        signer = DCISigner(
