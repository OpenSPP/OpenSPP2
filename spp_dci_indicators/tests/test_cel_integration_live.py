...
@tagged("post_install", "-at_install", "dci_integration")
class TestCELSymbolProvidersIntegration(TransactionCase):
    """Integration tests for CEL symbol providers with mock DCI server."""
    ...
    def setUpClass(cls):
        ...
        cls.partner = cls.env["res.partner"].create(
            {
                "name": "CEL Integration Test Partner",
                "is_registrant": True,
                "is_group": False,
            }
        )
        ...
        cls.dr_data_source = cls.env["spp.dci.data.source"].create(
            {
                "name": "Mock DR for CEL",
                "code": "mock_dr_cel",
                "base_url": cls.mock_url,
                "auth_type": "bearer",
                "bearer_token": "cel-test-token",
                "our_sender_id": "openspp.cel.test",
                "registry_type": "dr",
                "state": "active",
            }
        )
    ...
    def test_dr_symbol_provider_query_live(self):
        """Test DRSymbolProvider.query_live() with mock server."""
        ...
        # Mock the DRService since it requires specific setup
        with patch("odoo.addons.spp_dci_indicators.symbols.dci_symbols._get_data_source_by_type") as mock_get_ds:
            mock_get_ds.return_value = self.dr_data_source

            with patch("odoo.addons.spp_dci_client_dr.services.DRService") as mock_service_class:
                ...
                mock_service.sync_disability_data.assert_called_once_with(self.partner)
                self.assertTrue(result)

    def test_cel_integration_service_with_mock(self):
        """Test CEL integration service creates providers correctly."""
        ...
        # Verify providers are bound to correct partner
        self.assertEqual(symbols["dr"].partner, self.partner)
        self.assertEqual(symbols["crvs"].partner, self.partner)
        self.assertEqual(symbols["ibr"].partner, self.partner)
