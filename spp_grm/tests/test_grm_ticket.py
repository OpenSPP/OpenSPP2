...
class SPPGRMTicketTests(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.ticket_stage_open = cls.env["spp.grm.ticket.stage"].create({"name": "Open", "is_closed": False})
        cls.ticket_stage_closed = cls.env["spp.grm.ticket.stage"].create({"name": "Closed", "is_closed": True})
