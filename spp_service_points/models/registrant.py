...
class OpenSPPRegistrant(models.Model):
    ...
    individual_service_points_ids = fields.Many2many(
        comodel_name="spp.service.point",
        relation="service_point_ids_individual_ids_rel",
        column1="individual_id",
        column2="service_point_id",
        compute="_compute_individual_service_points_ids",
        inverse="_inverse_individual_service_points_ids",
        store=True,
    )

    @api.depends("parent_id")
    def _compute_individual_service_points_ids(self):
        for rec in self:
            rec.individual_service_points_ids.update_individual_ids()
            if rec.parent_id:
    ...
    def _inverse_individual_service_points_ids(self):
        pass
    ...
    def unlink(self):
        ...
        if child_ids:
            child_ids._compute_individual_service_points_ids()
...
class OpenSPPServicePoint(models.Model):
    ...
    service_type_ids = fields.Many2many(
        "spp.vocabulary.code",
        relation="spp_service_point_type_rel",
        column1="service_point_id",
        column2="service_type_id",
        string="Service Types",
        domain="[('namespace_uri', '=', 'urn:openspp:vocab:service-types')]",
    )
    ...
    def update_individual_ids(self):
        for res in self:
            ...
            res.individual_ids = [Command.set(child_ids)]
        ...
        for child_id in self.res_partner_company_id.child_ids:
            ...
            if child_id.email:
                try:
                    ...
                    _logger.error(
                        "User creation failed for partner_id=%s: %s",
                        child_id.id,
                        type(e).__name__,
                    )
                    raise UserError(
