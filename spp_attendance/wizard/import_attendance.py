...
class ImportAttendanceWiz(models.TransientModel):
    ...
    def action_import_attendance(self):
        ...
        if not r.ok:
            raise UserError(_("Authentication failed. Reason: {}.").format(r.reason))
        ...
        if not r.ok:
            raise UserError(_("Import failed. Reason: {}.").format(r.reason))
    ...
    def _import_attendance(self, data):
        ...
        if missing_required_fields:
            raise UserError(
                _("Please fill in the following required fields: {}").format(", ".join(missing_required_fields))
            )
        ...
        for element in elements:
            ...
            if not element.isdigit():
                if element not in data:
                    raise UserError(_("Element {} not found in data").format(element))
                data = data[element]
