...
class SppAttendanceController(Controller):
    ...
    def delete_attendance_list(self, ids=None, **kwargs):
        ...
        if not ids:
            ...
            try:
                ...
                _logger.warning("Invalid ids parameter format: expected comma-separated integers")
    ...
    def attendance_list(self, page=1, limit=30, last_id=None, **kwargs):
        if error := self.validate_request_header_and_body():
            return error
        ...
        # Use cursor-based pagination if last_id provided, otherwise fall back to offset
        if last_id:
            try:
                last_id = int(last_id)
            except ValueError:
                return self.error_wrapper(400, "last_id must be an integer.")
    ...
    def get_subscriber_list_information(self, page=1, limit=30, last_id=None, **kwargs):
        if error := self.validate_request_header_and_body():
            return error
        ...
        # Use cursor-based pagination if last_id provided
        if last_id:
            try:
                last_id = int(last_id)
            except ValueError:
