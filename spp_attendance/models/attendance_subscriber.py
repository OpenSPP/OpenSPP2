...
class AttendanceSubscriber(models.Model):
    ...
    partner_id = fields.Many2one("res.partner", required=True, ondelete="restrict", readonly=True)
    ...
    # Odoo 19 removed partner.mobile; mirror the main phone instead to keep API compatibility
    mobile = fields.Char(
        related="partner_id.phone", inherited=True, compute="_compute_partner", inverse="_inverse_partner", store=True
    )
    ...
    @api.depends("partner_id.name", "partner_id.email", "partner_id.phone")
                ...
                record.partner_id.write(
                    {
                        ...
                        "email": record.email,
                        "phone": record.phone or record.mobile,
                    }
                )
    ...
    def get_attendance_subscriber_info(self):
        self.ensure_one()
        return {
            # Use the external person identifier as the public ID instead of the database ID.
            "id": self.person_identifier,
            "person_id": self.person_identifier,
