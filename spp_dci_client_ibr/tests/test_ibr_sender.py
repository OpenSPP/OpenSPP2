...
"""Tests for IBR Sender Registry model."""
...
class TestIBRSender(IBRClientCommon):
    """Test cases for IBR Sender Registry model."""
    ...
    def test_create_sender_with_public_key(self):
        """Test creating a IBR sender with public key."""
        sender = self.create_test_ibr_sender(
            sender_id="ibr.test.gov",
            name="Test IBR Registry",
        )

        self.assertTrue(sender.id, "Sender should be created")
        self.assertEqual(sender.sender_id, "ibr.test.gov")
        self.assertEqual(sender.name, "Test IBR Registry")
    ...
    def test_create_sender_minimal(self):
        """Test creating a IBR sender with minimal required fields."""
        sender = self.IBRSender.create(
            {
                "name": "Minimal IBR",
                "sender_id": "ibr.minimal.gov",
            }
        )

        self.assertTrue(sender.id, "Sender should be created")
        self.assertEqual(sender.sender_id, "ibr.minimal.gov")
    ...
    def test_get_by_sender_id_found(self):
        ...
        sender = self.create_test_ibr_sender(sender_id="ibr.lookup.test")

        result = self.IBRSender.get_by_sender_id("ibr.lookup.test")

        self.assertEqual(result.id, sender.id, "Should return the correct sender")
        self.assertEqual(result.sender_id, "ibr.lookup.test")

    def test_get_by_sender_id_not_found(self):
        """Test lookup by sender_id returns empty recordset when not found."""
        result = self.IBRSender.get_by_sender_id("ibr.nonexistent.gov")
    ...
    def test_get_by_sender_id_inactive_not_returned(self):
        ...
        self.create_test_ibr_sender(
            sender_id="ibr.inactive.test",
            active=False,
        )

        result = self.IBRSender.get_by_sender_id("ibr.inactive.test")
    ...
    def test_sender_id_uniqueness_constraint(self):
        ...
        self.create_test_ibr_sender(sender_id="ibr.unique.test")
    ...
    def test_jwks_url_format_validation(self):
        ...
        sender = self.IBRSender.create(
            {
                "name": "Test JWKS",
                "sender_id": "ibr.jwks.test",
                "jwks_url": "https://ibr.test.gov/.well-known/jwks.json",
            }
        )
        self.assertTrue(sender.id, "Should accept valid HTTPS URL")

        sender2 = self.IBRSender.create(
            {
                "name": "Test JWKS HTTP",
                "sender_id": "ibr.jwks.test2",
    ...
    def test_fetch_public_key_without_url(self):
        ...
        sender = self.create_test_ibr_sender(jwks_url=None)
    ...
    @patch("odoo.addons.spp_dci_client_ibr.models.ibr_sender.requests.get")
    def test_fetch_public_key_success(self, mock_get):
        ...
        sender = self.IBRSender.create(
            {
                "name": "Test Fetch",
                "sender_id": "ibr.fetch.test",
                "jwks_url": "https://ibr.fetch.test/.well-known/jwks.json",
            }
        )
        ...
        mock_response.json.return_value = {
            "keys": [
                {
                    "kid": "ibr.fetch.test|2024-key|ed25519",
                    "kty": "OKP",
                    "crv": "Ed25519",
        ...
        sender = self.IBRSender.create(
            {
                ...
                "sender_id": "ibr.httperror.test",
                "jwks_url": "https://ibr.error.test/jwks",
            }
        )
    ...
    def test_fetch_public_key_invalid_jwks(self, mock_get):
        ...
        sender = self.IBRSender.create(
            {
                "name": "Test Invalid JWKS",
                "sender_id": "ibr.invalid.test",
                "jwks_url": "https://ibr.invalid.test/jwks",
            }
        )
    ...
    def test_fetch_public_key_no_matching_key(self, mock_get):
        ...
        sender = self.IBRSender.create(
            {
                "name": "Test No Match",
                "sender_id": "ibr.nomatch.test",
                "jwks_url": "https://ibr.nomatch.test/jwks",
            }
        )
    ...
    def test_action_fetch_public_key_success(self, mock_get):
        ...
        sender = self.IBRSender.create(
            {
                "name": "Test Action",
                "sender_id": "ibr.action.test",
                "jwks_url": "https://ibr.action.test/jwks",
            }
        )
        ...
        mock_response.json.return_value = {
            "keys": [
                {
                    "kid": "ibr.action.test|2024-key|ed25519",
                    "kty": "OKP",
                    "crv": "Ed25519",
                    "x": "11qYAYKxCrfVS_7TyWQHOg7hcvPapiMlrwIaaPcHURo",
                }
        ...
        sender = self.IBRSender.create(
            {
                ...
                "sender_id": "ibr.actionerror.test",
                "jwks_url": "https://ibr.actionerror.test/jwks",
            }
        )
    ...
    def test_multiple_senders_with_different_algorithms(self):
        ...
        sender1 = self.IBRSender.create(
            {
                "name": "Ed25519 Sender",
                "sender_id": "ibr.ed25519.test",
                "algorithm": "ed25519",
            }
        )
    ...
    def test_sender_notes_field(self):
        ...
        sender = self.create_test_ibr_sender()
        test_notes = "This is a test IBR registry for integration testing."
    ...
    def test_sender_ordering(self):
        ...
        self.IBRSender.create(
            {
                "name": "Zebra IBR",
                "sender_id": "ibr.zebra.test",
            }
        )
        self.IBRSender.create(
            {
                "name": "Alpha IBR",
                "sender_id": "ibr.alpha.test",
            }
        )

        senders = self.IBRSender.search([("sender_id", "like", "ibr.%.test")])

        self.assertEqual(senders[0].name, "Alpha IBR", "Should be ordered by name")
