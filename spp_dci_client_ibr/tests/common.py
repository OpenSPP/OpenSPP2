...
"""Common test utilities for DCI Client IBR module."""
...
class IBRClientCommon(TransactionCase):
    """Base test class with shared utilities for IBR Client tests."""
    ...
    def setUpClass(cls):
        ...
        # Create test keypair for IBR sender (registry side)
        ...
        # Test IBR sender configuration
        cls.test_sender_id = "ibr.national.gov"
        cls.test_key_id = "2024-test-key"
        cls.test_algorithm = "ed25519"

        # Models
        cls.IBRSender = cls.env["spp.dci.ibr.sender"]

    @classmethod
    def create_test_ibr_sender(
        cls,
        sender_id=None,
        name=None,
        public_key=None,
        ...
        """Helper to create a test IBR sender registry entry.
        ...
        Returns:
            spp.dci.ibr.sender: Created sender record
        """
        sender_id = sender_id or cls.test_sender_id
        return cls.IBRSender.create(
            {
                "name": name or f"Test IBR Sender {sender_id}",
                "sender_id": sender_id,
    ...
    def create_signed_envelope(
        cls,
        sender_id=None,
        receiver_id="ibr.client.openspp",
        action="notify",
        message=None,
    ):
        """Helper to create a signed DCI envelope for testing IBR callbacks.

        Args:
            sender_id: Sender ID (defaults to cls.test_sender_id)
            receiver_id: Receiver ID
            action: DCI action (default: 'notify' for callbacks)
            message: Message payload (defaults to IBR birth event)
        ...
        header_model = DCIMessageHeader(
            version="1.0.0",
            message_id="ibr-callback-001",
            message_ts=datetime.now(UTC),
            action=action,
            sender_id=sender_id,
            sender_uri="https://ibr.national.gov/api/v1",
        ...
        # Sign the message using IBR sender's private key
        signer = DCISigner(
