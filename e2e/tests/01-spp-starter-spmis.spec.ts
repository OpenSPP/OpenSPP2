...
async function installApp(page: Page, technicalName: string) {
  ...
  await page.locator('.o_loading').waitFor({ state: 'hidden', timeout: 180_000 }).catch(() => {});

  // Wait for at least one nav menu item to appear
  await expect(page.locator('.o_main_navbar .o_menu_sections .o_nav_entry').first()).toBeVisible({ timeout: 180_000 });
...
test.describe.serial('OpenSPP Starter SP-MIS', () => {
  ...
  test('01 - login and install OpenSPP Starter SP-MIS', async () => {
    ...
    console.log('⏳ Giving the system 30 seconds to fully initialize all menu items before proceeding...');
    await page.waitForTimeout(30_000);

    await expect(page.locator('.o_main_navbar')).toBeVisible();
    const menuItems = page.locator('.o_main_navbar .o_nav_entry, .o_main_navbar .o_dropdown');
  ...
  test('02 - create individual registrant', async () => {
    ...
    await page.getByRole('option', { name: 'Armed Forces Occupations', exact: true }).click();
    ...
    await expect(page.getByRole('textbox', { name: 'Given Name?' })).toHaveValue('John Miller');
    console.log('✅ Given Name: John Miller');

    await expect(page.getByRole('button', { name: 'Date of Birth?' })).toContainText('Jan 1, 1975');
    console.log('✅ Date of Birth: Jan 1, 1975');

    await expect(page.getByRole('combobox', { name: 'Gender' })).toHaveValue('Male');
    console.log('✅ Gender: Male');

    await expect(page.getByRole('combobox', { name: 'Civil Status?' })).toHaveValue('Married');
    ...
    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('johnmiller@mail.com');
    ...
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'DOE, JOHN MILLER' })).toBeVisible({ timeout: 10_000 });
    console.log('✅ Registrant "DOE, JOHN MILLER" verified in Browse All Individuals list');

    await page.getByRole('cell', { name: 'DOE, JOHN MILLER' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.locator('.o_form_view')).toBeVisible({ timeout: 10_000 });
    await expect(page.getByRole('textbox', { name: 'Family Name?' })).toHaveValue('Doe');
    console.log('✅ Individual registrant "DOE, JOHN MILLER" created via GUI successfully');
  });
  ...
  test('03 - create individual registrant: Reyes, Maria Clara', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Family Name?' })).toHaveValue('Reyes');
    await expect(page.getByRole('textbox', { name: 'Given Name?' })).toHaveValue('Maria Clara');
    await expect(page.getByRole('button', { name: 'Date of Birth?' })).toContainText('Mar 15, 1985');
    ...
    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('mariaclara@mail.com');
    ...
    await page.getByRole('menuitem', { name: 'All Individuals' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'REYES, MARIA CLARA' })).toBeVisible({ timeout: 10_000 });
    console.log('✅ Individual registrant "REYES, MARIA CLARA" created via GUI successfully');
  });
  ...
  test('04 - create individual registrant: Santos, Jose Miguel', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Family Name?' })).toHaveValue('Santos');
    await expect(page.getByRole('textbox', { name: 'Given Name?' })).toHaveValue('Jose Miguel');
    await expect(page.getByRole('button', { name: 'Date of Birth?' })).toContainText('Jul 22, 1990');
    await expect(page.getByRole('combobox', { name: 'Gender' })).toHaveValue('Male');
    await expect(page.getByRole('combobox', { name: 'Civil Status?' })).toHaveValue('Married');
    await expect(page.getByRole('textbox', { name: 'Income' })).toHaveValue('20,000.00');
    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('josemiguel@mail.com');
    ...
    await page.getByRole('menuitem', { name: 'All Individuals' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'SANTOS, JOSE MIGUEL' })).toBeVisible({ timeout: 10_000 });
    console.log('✅ Individual registrant "SANTOS, JOSE MIGUEL" created via GUI successfully');
  });
  ...
  test('05 - create individual registrant: Garcia, Ana Liza', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Family Name?' })).toHaveValue('Garcia');
    await expect(page.getByRole('textbox', { name: 'Given Name?' })).toHaveValue('Ana Liza');
    await expect(page.getByRole('button', { name: 'Date of Birth?' })).toContainText('Nov 8, 1968');
    await expect(page.getByRole('combobox', { name: 'Gender' })).toHaveValue('Female');
    await expect(page.getByRole('combobox', { name: 'Civil Status?' })).toHaveValue('Married');
    await expect(page.getByRole('textbox', { name: 'Income' })).toHaveValue('8,000.00');
    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('analiza@mail.com');
    ...
    await page.getByRole('menuitem', { name: 'All Individuals' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'GARCIA, ANA LIZA' })).toBeVisible({ timeout: 10_000 });
    console.log('✅ Individual registrant "GARCIA, ANA LIZA" created via GUI successfully');
  });
  ...
  test('06 - create individual registrant: Cruz, Roberto Paolo', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Given Name?' })).toHaveValue('Roberto Paolo');
    await expect(page.getByRole('button', { name: 'Date of Birth?' })).toContainText('Apr 30, 1978');
    await expect(page.getByRole('combobox', { name: 'Gender' })).toHaveValue('Male');
    await expect(page.getByRole('combobox', { name: 'Civil Status?' })).toHaveValue('Married');
    await expect(page.getByRole('textbox', { name: 'Income' })).toHaveValue('12,000.00');
    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('robertopaolo@mail.com');
    ...
    await page.getByRole('menuitem', { name: 'All Individuals' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'CRUZ, ROBERTO PAOLO' })).toBeVisible({ timeout: 10_000 });
    console.log('✅ Individual registrant "CRUZ, ROBERTO PAOLO" created via GUI successfully');
  });
  ...
  test('07 - create group registrant', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Group Name?' })).toHaveValue('Doe Family');
    console.log('✅ Group Name: Doe Family');

    await expect(page.getByRole('combobox', { name: 'Group Type?' })).toHaveValue('Family');
    console.log('✅ Group Type: Family');

    await expect(page.getByRole('textbox', { name: 'Address' })).toHaveValue('Sample Address');
    console.log('✅ Address: Sample Address');

    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('Doefamiliy@mail.com');
    ...
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'Doe Family' })).toBeVisible({ timeout: 10_000 });
  ...
  test('08 - create group registrant: Santos Family', async () => {
    ...
    await expect(page.getByRole('textbox', { name: 'Group Name?' })).toHaveValue('Santos Family');
    console.log('✅ Group Name: Santos Family');

    await expect(page.getByRole('combobox', { name: 'Group Type?' })).toHaveValue('Family');
    console.log('✅ Group Type: Family');

    await expect(page.getByRole('textbox', { name: 'Address' })).toHaveValue('456 Rizal Street');
    console.log('✅ Address: 456 Rizal Street');

    await expect(page.getByRole('textbox', { name: 'Email?' })).toHaveValue('santosfamily@mail.com');
    ...
    await page.getByRole('menuitem', { name: 'All Groups' }).click();
    await page.waitForLoadState('domcontentloaded');
    await expect(page.getByRole('cell', { name: 'Santos Family' })).toBeVisible({ timeout: 10_000 });
  ...
  test('10 - create areas', async () => {
    ...
    await page.getByRole('button', { name: 'Save manually' }).click();
    await expect(page.getByRole('textbox', { name: 'Enter Area Name...' })).toHaveValue('Philippines');
    ...
    await page.getByRole('button', { name: 'Save manually' }).click();
    await expect(page.getByRole('textbox', { name: 'Enter Area Name...' })).toHaveValue('Manila');
    ...
    await page.getByRole('textbox', { name: 'Area (km²)?' }).fill('5000');
    await page.getByRole('button', { name: 'Save manually' }).click();
    await expect(page.getByRole('textbox', { name: 'Enter Area Name...' })).toHaveValue('Cebu');
    ...
    await page.getByRole('button', { name: 'Save manually' }).click();
    await expect(page.getByRole('textbox', { name: 'Enter Area Name...' })).toHaveValue('Mandaue City');
  ...
  test('13 - configure approval definition for change requests', async () => {
    ...
    await page.getByRole('button', { name: 'Save manually' }).click();
    await expect(page.getByRole('textbox', { name: 'Name' })).toHaveValue('Change request');
    console.log('✅ Approval Definition saved: Change request (Model: Change Request, Group: Change Requests / Validator HQ)');
    ...
    const changeRequestTypes = ['Edit Individual Information', 'Edit Group Information', 'Update ID Document'];
    ...
    for (const typeName of changeRequestTypes) {
      ...
      await page.getByRole('button', { name: 'Save manually' }).click();
      await expect(page.getByRole('combobox', { name: 'Approval Workflow' })).toHaveValue('Change request');
      console.log(`✅ Approval Workflow "Change request" (HQ validator) assigned to ${typeName}`);
  ...
  test('14 - define CR document types for the Philippines (manual vocabulary codes)', async () => {
    ...
    const documentTypes = [
      ...
      { code: 'barangay_residency_certificate', display: 'Barangay Certificate of Residency' },
