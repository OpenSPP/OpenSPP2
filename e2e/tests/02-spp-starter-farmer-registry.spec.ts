...
test.describe('OpenSPP', () => {
  ...
  test('login and install OpenSPP Starter Farmer Registry, then verify nav menus', async ({ page }) => {
    await login(page);
    ...
    await page.screenshot({ path: 'reports/post-install-farmer-registry-navbar.png', fullPage: false });
  });
});
