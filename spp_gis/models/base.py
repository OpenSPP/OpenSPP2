...
class Base(models.AbstractModel):
    ...
    def get_gis_layers(self, view_id=None, view_type="gis", **options):
        ...
        # Create a mapping of layer IDs to layer records for choropleth config
        layer_records = {layer.id: layer for layer in view.data_layer_ids}
        ...
        for layer_dict in layer_dict_list:
            ...
            # Add choropleth configuration if this is a choropleth layer
            layer_record = layer_records.get(layer_dict["id"])
            if layer_record and layer_dict.get("geo_repr") == "choropleth":
                layer_dict["choropleth_config"] = layer_record._get_choropleth_config()

            # Add layer domain filter
            if layer_record and layer_record.domain:
                layer_dict["domain"] = layer_record.domain
