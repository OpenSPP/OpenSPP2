...
export class LayersPanel extends Component {
    ...
    async loadIsAdmin() {
        try {
            const result = await this.user.hasGroup("spp_gis.group_gis_admin");
    ...
    async loadLayers() {
        try {
            ...
            this.state.gisLayers = {
                ...
                default_zoom: result?.default_zoom,
                ...result,
            };
        } catch (error) {
            console.error("Error loading layers:", error);
            // Keep default empty arrays on error
            this.state.gisLayers = {actives: [], backgrounds: []};
        }
    }
    ...
    async onEditButtonSelected(dataLayer) {
        await this.openFormViewDialog("spp_gis.action_spp_gis_data_layer_view", {
    ...
    async onEditRasterButtonSelected(layer) {
        await this.openFormViewDialog("spp_gis.action_spp_gis_raster_layer_form", {
...
LayersPanel.template = "spp_gis.LayersPanel";
LayersPanel.props = {
