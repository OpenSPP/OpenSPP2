    ...
    def test_cross_env_consistency(self):
        ...
        with self._get_mocked_request():
            with new_rollbacked_env() as env2:
                ...
                self.assertEqual(env1["ir.http"]._endpoint_route_last_version(), last_version0)
                self.assertEqual(env2["ir.http"]._endpoint_route_last_version(), last_version0)
                rmap = self.env["ir.http"].routing_map()
                self.assertIn(route, [x.rule for x in rmap._rules])
                ...
                rmap = env2["ir.http"].routing_map()
                self.assertIn(route, [x.rule for x in rmap._rules])
                self.assertTrue(env1["ir.http"]._endpoint_route_last_version() > last_version0)
                self.assertTrue(env2["ir.http"]._endpoint_route_last_version() > last_version0)
