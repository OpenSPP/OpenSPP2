...
class TestRegistry(TransactionCase):
    ...
    def _count_rules(self, groups=("test_route_handler",)):
        ...
        self.env.cr.execute("SELECT COUNT(id) FROM endpoint_route WHERE route_group IN %s", (groups,))
    ...
    def test_get_rules(self):
        ...
        self.assertEqual([x.key for x in self.reg.get_rules()], ["route1", "route2", "route3"])
    ...
    def test_update_rule(self):
        rule1, rule2 = self._make_rules(stop=3)
        self.assertEqual(self.reg._get_rule("route1").handler_options.method_name, "handler1")
        self.assertEqual(self.reg._get_rule("route2").handler_options.method_name, "handler1")
        rule1.options = {
            "handler": {
                "klass_dotted_path": CTRLFake._path,
                "method_name": "handler2",
        ...
        self.assertEqual(self.reg._get_rule("route1").handler_options.method_name, "handler2")
        self.assertEqual(self.reg._get_rule("route2").handler_options.method_name, "handler3")
