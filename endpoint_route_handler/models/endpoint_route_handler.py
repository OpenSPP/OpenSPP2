...
class EndpointRouteHandler(models.AbstractModel):
    ...
    auth_type = fields.Selection(selection="_selection_auth_type", default="user_endpoint")
    ...
    request_method = fields.Selection(selection="_selection_request_method", required=True)
    ...
    endpoint_hash = fields.Char(compute="_compute_endpoint_hash", help="Identify the route with its main params")
    ...
    def _check_route_unique_across_models(self):
        ...
        for model in all_models:
            if model != self._name and self.env[model].sudo().search_count([("route", "in", routes)]):
    ...
    def _get_endpoint_route_consumer_models(self):
        ...
        for model in self.env.values():
            if model._name != route_model and not model._abstract and route_model in model._inherit:
    ...
    def _compute_endpoint_hash(self):
        ...
        values = [{fname: rec[fname] for fname in self._routing_impacting_fields()} for rec in self]
    ...
    def _check_route(self):
        for rec in self:
            if rec.route in self._blacklist_routes:
                raise exceptions.UserError(
                    _("`%(name)s` uses a blacklisted routed = `%(route)s`") % {"name": rec.name, "route": rec.route}
                )
    ...
    def _check_request_method(self):
        for rec in self:
            if rec.request_method in ("POST", "PUT") and not rec.request_content_type:
                raise exceptions.UserError(_("Request content type is required for POST and PUT."))
    ...
    def _register_single_controller(self, options=None, key=None, init=False):
        ...
        self._logger.debug("Registered controller %s (auth: %s)", self.route, self.auth_type)
