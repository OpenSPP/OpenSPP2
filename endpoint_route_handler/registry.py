...
def query_multi_update(cr, table_name, rows, cols):
    ...
    template = sql.SQL("(") + sql.SQL(",").join([sql.SQL(f"%({col})s") for col in cols]) + sql.SQL(")")
    query = sql.SQL(
        """
...
class EndpointRule:
    ...
    def __init__(self, dbname, key, route, options, routing, endpoint_hash, route_group=None):
        self._dbname = dbname
        self.key = key
        self.route = route
