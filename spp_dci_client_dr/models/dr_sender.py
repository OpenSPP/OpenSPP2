...
"""DR Sender Registry for signature verification.

This model stores trusted DR registry public keys for verifying
incoming callback notifications. Similar to spp.dci.crvs.sender
but specifically for Disability Registry client-side verification.
"""
...
class DRSenderRegistry(models.Model):
    """DR Sender Registry.

    Stores public keys of trusted Disability Registry systems for signature
    verification of incoming callback notifications.
    """

    _name = "spp.dci.dr.sender"
    _description = "DR Sender Registry"
    _order = "name"

    name = fields.Char(
        required=True,
        string="Name",
        help="Friendly name for this Disability Registry",
    )
    sender_id = fields.Char(
        required=True,
        string="Sender ID",
        help="DCI sender identifier for the DR registry (e.g., 'dr.national.gov')",
        index=True,
    )
    ...
    jwks_url = fields.Char(
        string="JWKS URL",
        help="URL to DR registry's JWKS endpoint (/.well-known/jwks.json)",
    )
    ...
    notes = fields.Text(
        string="Notes",
        help="Additional notes about this Disability Registry",
    )

    @api.constrains("sender_id")
    def _check_sender_id_unique(self):
        """Ensure sender_id is unique across all DR sender entries."""
        for record in self:
            if record.sender_id:
                ...
                if duplicate:
                    raise ValidationError(_("Sender ID must be unique. A DR sender already exists with this ID."))
    ...
    def action_fetch_public_key(self):
        """Fetch public key from DR registry's JWKS endpoint (button action).
        ...
        try:
            ...
            _logger.error(
                "Failed to fetch public key for DR sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
                exc_info=True,
    ...
    def fetch_public_key(self):
        """Fetch public key from DR registry's JWKS endpoint.
        ...
        if not self.jwks_url:
            raise UserError(_("JWKS URL not configured for DR sender %s") % self.sender_id)

        _logger.info(
            "Fetching public key for DR sender %s from %s",
            self.sender_id,
            self.jwks_url,
        )
        ...
        try:
            ...
            if not matching_key:
                raise UserError(
                    _("No matching key found in JWKS for DR sender %s. Expected kid to start with '%s|'")
            ...
            _logger.info(
                "Successfully fetched public key for DR sender %s (algorithm: %s)",
                self.sender_id,
                algorithm,
            )
            ...
            _logger.error(
                "HTTP error fetching JWKS for DR sender %s from %s: %s",
                self.sender_id,
                self.jwks_url,
                str(e),
            )
    ...
    def get_verifier(self):
        """Return a DCIVerifier instance for this DR sender.
        ...
        if not self.public_key:
            raise UserError(
                _("No public key available for DR sender %s. Please fetch the public key first.") % self.sender_id
            )

        if not self.algorithm:
            raise UserError(_("Algorithm not set for DR sender %s") % self.sender_id)
    ...
    def get_by_sender_id(self, sender_id):
        """Lookup DR sender by sender_id.

        Args:
            sender_id: DCI sender identifier

        Returns:
            spp.dci.dr.sender: Sender record or empty recordset
        """
