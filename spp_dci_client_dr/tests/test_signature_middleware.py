...
"""Tests for DR DCI signature verification middleware."""
...
class TestDRSignatureMiddleware(DRClientCommon):
    """Test cases for DR signature verification middleware."""

    def setUp(self):
        """Set up test fixtures for each test method."""
        super().setUp()

        # Create a test DR sender in the registry
        self.test_sender = self.create_test_dr_sender()
    ...
    async def _verify_signature(self, envelope_data):
        """Helper to call the verify_dr_signature function.
        ...
        """
        from odoo.addons.spp_dci_client_dr.middleware.signature import (
            verify_dr_signature,
        )
        ...
        return await verify_dr_signature(envelope, self.env)
    ...
    def test_verify_logs_success(self):
        ...
        with patch("odoo.addons.spp_dci_client_dr.middleware.signature._logger") as mock_logger:
            # Run async verification
            import asyncio
    ...
    def test_verify_with_different_message_types(self):
        """Test verification with different DR event types."""
        import asyncio
    ...
    def test_verify_sender_lookup_uses_sudo(self):
        ...
        # the current user doesn't have read access to spp.dci.dr.sender
