...
- fix: case creation from an escalation rule never worked (it passed a field ``spp.case`` does
  ...
  the ticket assignee or the rule owner as case worker. When neither is a real active user —
  an unassigned ticket under a superuser-owned rule resolves to ``__system__`` — the escalation
  is refused and rolled back rather than filing a case with OdooBot as the worker responsible;
  the message names the ticket and the remediation.
...
- fix: rule CEL validation reports a bad expression as a ``ValidationError`` whether the parser
  raises ``SyntaxError`` or ``RecursionError`` (nesting past its depth limit); previously only
  ``SyntaxError`` was caught. An unexpected parser failure is no longer reported as the user's
  own invalid expression with the traceback discarded: it is logged with its traceback and
