...
# Fields the preset insists on every install/upgrade. Anything else on
# the variable (labels, descriptions, category) is left to whoever last
# edited it.
...
def post_init_hook(env):
    ...
    bridge dispatcher can route it to the OpenSPP-DR instance.

    Why this exists vs. just trusting the data XML override:

      1. The data XML uses noupdate="1", which Odoo honours by setting
         noupdate=True on the ir.model.data entry. On subsequent
         upgrades of THIS module, the XML is skipped — but operators
         may have clobbered the binding manually, or another module's
         data load may have reset it. The hook is the one place that
         always runs on -u and can restore drift.

      2. spp_studio's standard_variables.xml creates the record in
         DRAFT state by default. The preset must explicitly activate it
         so it participates in the cache pre-warm (`active=True`) and
         in the CEL resolver's symbol lookup (`state='active'`). The
         XML data load doesn't reliably push it through the state
         machine.
    ...
    provider = env.ref(
        "spp_dci_openspp_dr.openspp_dr_provider",
        raise_if_not_found=False,
    )
    if not provider:
        _logger.error(
            "spp_dci_openspp_dr.openspp_dr_provider not found; cannot "
            "re-assert DCI binding on has_disability. Verify "
            "data/openspp_dr_data_provider.xml loaded successfully."
        )
        return
    ...
    expected = {
        ...
        # State + active control whether the variable participates in
        # the resolver / precompute pipeline:
    ...
    if drift:
        ...
        # _pre_activate would reject draft -> active if source_type is
        # 'field' and the field is missing; we're flipping source_type
