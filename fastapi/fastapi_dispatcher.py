...
class FastApiDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        with fastapi_app_pool.get_app(env=request.env, root_path=path) as app:
            ...
            with self._manage_odoo_env(uid):
                ...
                return self.request.make_response(data.getvalue(), headers=self.headers, status=self.status)

    def handle_error(self, exc):
        headers = getattr(exc, "headers", None)
        status_code, body = convert_exception_to_status_body(exc)
        return self.request.make_json_response(body, status=status_code, headers=headers)
    ...
    def _manage_odoo_env(self, uid=None):
        ...
        context = env.context
        if accept_language:
            lang = env["res.lang"].sudo()._get_lang_from_accept_language(accept_language)
            if lang:
