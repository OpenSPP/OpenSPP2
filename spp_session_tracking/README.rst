...
..
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   !! source digest: sha256:0052aade747981ffc847a1ce602c9d3db21acad2d8c6a2d124a208df04eb28ee
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
3. Add or modify session types as needed; topics are managed within each
   session type form when topic tracking is enabled
...
|                                                | types/topics; create/write own   |
|                                                | facilitated or co-facilitated    |
|                                                | sessions; read/write/create      |
|                                                | attendance for own sessions (no  |
|                                                | delete)                          |
+------------------------------------------------+----------------------------------+
...
The session user group can view all sessions but only edit sessions
where they are the facilitator or co-facilitator (via record rules).
Session creation requires manager access. The
``spp_security.group_spp_admin`` group implies manager access.
Multi-company record rules ensure users only see sessions belonging to
their company.
...
This guide covers all testable areas of the ``spp_session_tracking``
module. It is organized by functional area, with step-by-step
...
+=============+==========+===========+==========+==============+=========+
| Training    | TRAINING | Monthly   | 3.0h     | 80%          | Yes     |
| Session     |          |           |          |              |         |
+-------------+----------+-----------+----------+--------------+---------+
| Group       | MEETING  | Bi-weekly | 2.0h     | 75%          | No      |
| Meeting     |          |           |          |              |         |
+-------------+----------+-----------+----------+--------------+---------+
| Family      | FDS      | Monthly   | 2.5h     | 85%          | Yes     |
| Development |          |           |          |              |         |
| Session     |          |           |          |              |         |
+-------------+----------+-----------+----------+--------------+---------+
...
**Expected**: 4 topics exist: Nutrition and Health, Education and Child
Development, Financial Literacy, Positive Parenting.

3. Open the **Training Session** type

**Expected**: 2 topics exist: Livelihood Skills, Basic Business
Management.
...
**Expected**: "Session Tracking" menu item is visible with the module
icon.
...
**Expected**: The "Configuration" submenu is NOT visible. Only
"Sessions" submenu is shown.
...
**Expected**: The archived type shows as inactive (greyed out in list
view with muted decoration).
...
**Expected**: The button shows the correct count of sessions for this
type.
...
**Expected**: List displays columns: Name, Code, Frequency, Duration,
Required Attendance %, Sessions, Active. Inactive types are greyed out.
...
**Expected**: The Configuration menu is not visible. If accessed via
URL, an access error is raised.
...
**Expected**: An access error is raised. Session users cannot create
sessions.
...
- **Header**: Action buttons (Start/Complete/Cancel based on state) +
  statusbar
- **Ribbons**: Green "Completed" ribbon on completed sessions, red
  "Cancelled" ribbon on cancelled
- **Button Box**: "Attended" stat button with user icon (shows
  attendance count)
- **Title**: Session name in large text
- **Main Info**: Two-column layout

  - Left: Session Type, Date, Location, Area
  - Right: Facilitator, Co-Facilitators, Start Time, End Time, Duration,
    Max Participants, Company
...
1. Open a session with type "Family Development Session"
   (track_topics=True)
...
**Expected**: A popup list opens showing all attendance records for this
session.
...
- All fields become read-only (name, type, date, location, participants,
  attendance, etc.)
...
- A confirmation dialog appears: "Are you sure you want to cancel this
  session?"
- Click OK
...
These transitions should NOT be possible via the UI (buttons are
hidden), but verify:
...
**Expected**: Buttons are hidden for invalid transitions. The form is
fully read-only for Completed and Cancelled sessions.

**5.6 Chatter Tracking**

1. Perform any state transition on a session

**Expected**: The chatter logs the state change (e.g., "State: Scheduled
-> In Progress"). Changes to Name, Session Type, Date, and Facilitator
are also tracked.
...
**Expected**: Attendance record is created. Row is highlighted green
(attended).
...
**Expected**: Attendance Rate shows 0.0% (no expected participants to
calculate against).
...
**Expected**: Error: "A participant can only have one attendance record
per session."
...
**Expected**: The attendance list is read-only. No "Add a line" button
is available.
...
- Columns: Date, Name, Session Type, Facilitator, Location, Attendance
  Count, Attendance Rate, State
...
1. In the list view, click the column selector (gear icon or right-click
   header)
2. Toggle Location off, Area on

**Expected**: Location column hides, Area column appears. Settings
persist within the session.

**7.3 Calendar View**

1. Switch to Calendar view (click the calendar icon in the view
   switcher)
...
- Clicking a date does NOT open a quick-create popup (quick_create is
  disabled)
...
- Cards are grouped by State (Scheduled, In Progress, Completed,
  Cancelled columns)
- Each card shows: Name (bold), Session Type, Date, Facilitator,
  Attendance Rate %
...
- Cards cannot be dragged between columns (state changes via buttons
  only)
...
**Expected**: A bar chart showing Duration Hours grouped by Session
Type.
...
**Expected**: A pivot table with Session Type as rows, Facilitator as
columns, and Duration Hours as the measure.

**7.7 My Sessions View**

1. Go to **Session Tracking > Sessions > My Sessions**

**Expected**: Only sessions where the current user is the facilitator
are shown (My Sessions filter is pre-applied).
...
**Expected**: Only your scheduled sessions are shown. Filters combine
with AND logic.
...
**Expected**: All sessions are visible (including ones facilitated by
others), regardless of facilitator.
...
1. Log in as **Session User**
2. Open a session where you are a **co-facilitator** (but not the main
   facilitator)
3. Edit the Location field and save
...
**Expected**: Access error is raised. You cannot modify sessions you
don't facilitate.
...
**Expected**: Access error. You cannot create attendance on sessions you
don't facilitate.
...
1. Log in as **Session Manager**
2. Create, read, update, and delete sessions, session types, topics, and
   attendance

**Expected**: All operations succeed. Managers have full CRUD on
everything.
...
**Expected**: Only sessions belonging to Company A (or no company) are
visible. Sessions belonging to Company B are NOT visible.
...
**Expected**: Error preventing deletion: the session type cannot be
deleted while sessions reference it (ondelete=restrict).
...
**Expected**: Error preventing deletion: the participant cannot be
deleted while attendance records reference them (ondelete=restrict).
...
**Expected**: If spp_case_session is installed, a "Cases" stat button
appears alongside the "Attended" stat button, inside the same button
box. There should NOT be duplicate button boxes.
...
**Expected**: Attendance Count = 0, Attendance Rate = 0.0%. Session
functions normally.
...
**Expected**: Both are created successfully. The unique constraint
allows multiple blank codes.
